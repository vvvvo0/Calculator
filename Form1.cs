...
namespace Calculator
{
    public partial class Form1 : Form
    {
        ...
        Operators currentOperator = Operators.None;// 열거형 변수로 currentOperator를 사용.
                                                   // 열거형 변수에 값을 넣을 때는 Operators.None,
                                                   // Operators.Add 등과 같이 열거형 내부에 있는 이름을 호출해 사용.
                                                   // 초기에는 연산자가 선택되지 않은 상태를 표현하기 위해 Operators.None으로 값을 초기화.
                                                   // (현재 연산자를 저장할 변수)
        Boolean operatorChangeFlag = false; // (연산자가 변경되었는지 여부를 저장할 변수)
                                            // operatorChangeFlag 변수는 연산자가 눌렸음을 알 수 있는 변수로,
                                            // 연산자가 눌리거나 변경되면 true 값을 갖습니다.
                                            // 연산자 버튼이 눌리면 기존에 입력한 숫자를 저장하고, 새로 숫자를 입력받기 위한 변수.
        int firstOperand = 0; // (첫 번째 피연산자를 저장할 변수)
                              // 피연산자

        int secondOperand = 0; // (두 번째 피연산자를 저장할 변수)
                               // 피연산자
